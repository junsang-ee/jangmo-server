...
public abstract class Criteria {
	 ...
	 * @return {@literal null} if {@code value} is null
	 */
	protected static <T extends Number & Comparable<T>> Predicate gt(
		@NonNull NumberPath<T> path, T value
	) {
	...
	protected static <T extends Comparable<T> & Temporal> Predicate goe(
		@NonNull TemporalExpression<T> path, T value
	) {
		return optional(value, path::goe);
	}
	...
	protected static <I, O> Set<O> toSet(
		Collection<? extends I> values, @NonNull Function<I, O> map
	) {
		if (isEmpty(values)) {
			return null;
		}
